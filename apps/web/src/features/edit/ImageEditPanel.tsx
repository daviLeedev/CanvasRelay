...
export function ImageEditPanel({
  ...
  templateJob?: ImageJobResponse | null;
  ...
  const initialJob = templateJob?.settings.operation === "edit" ? templateJob : job;
  const [source, setSource] = useState<File | null>(null);
  const [faceReference, setFaceReference] = useState<File | null>(null);
  const restoredEdit = initialJob?.settings.operation === "edit" ? initialJob.settings.edit : null;
  const [prompt, setPrompt] = useState(() => initialJob?.settings.operation === "edit" ? initialJob.prompt : "");
  const [aspectRatio, setAspectRatio] = useState<ImageEditJobCreate["aspectRatio"]>(
    () => initialJob?.settings.operation === "edit" ? initialJob.settings.aspectRatio : "4:3",
  );
  const [style, setStyle] = useState<ImageEditJobCreate["style"]>(
    () => initialJob?.settings.operation === "edit" ? initialJob.settings.style : "editorial",
  );
  const [seed, setSeed] = useState(() => initialJob?.settings.operation === "edit" ? String(initialJob.settings.seed) : "");
  const [steps, setSteps] = useState(() => restoredEdit?.steps ?? 8);
  const [cfg, setCfg] = useState(() => restoredEdit?.cfg ?? 1);
  const [referenceInfluence, setReferenceInfluence] = useState(() => restoredEdit?.referenceInfluence ?? 4);
  const [groundingResolution, setGroundingResolution] = useState(() => restoredEdit?.groundingResolution ?? 768);
  const [fitMode, setFitMode] = useState<ImageEditJobCreate["fitMode"]>(() => restoredEdit?.fitMode ?? "fit");
  const [sampler, setSampler] = useState(() => restoredEdit?.sampler ?? "euler");
  const [scheduler, setScheduler] = useState(() => restoredEdit?.scheduler ?? "simple");
  const [loras, setLoras] = useState<ImageEditLoraSelection[]>(() => restoredEdit?.loras ?? []);
        ...
        <details className={styles.advanced}>
          ...
          <div className={styles.advancedBody}>
            <div className={styles.settingGrid}>
              ...
              <Field label="Sampler" htmlFor="edit-sampler"><Select id="edit-sampler" value={selectedSampler} onChange={(event) => setSampler(event.target.value)} disabled={!options?.samplers.length}>{options?.samplers.map((item) => <option key={item}>{item}</option>) ?? <option>{selectedSampler}</option>}</Select></Field>
              <Field label="Scheduler" htmlFor="edit-scheduler"><Select id="edit-scheduler" value={selectedScheduler} onChange={(event) => setScheduler(event.target.value)} disabled={!options?.schedulers.length}>{options?.schedulers.map((item) => <option key={item}>{item}</option>) ?? <option>{selectedScheduler}</option>}</Select></Field>
            </div>
