...
@pytest.mark.anyio
async def test_comfyui_preserves_retryable_restart_error_after_recovery_grace(
    tmp_path: Path,
) -> None:
    workflow_path = tmp_path / "workflow.json"
    write_workflow(workflow_path)
    ...
    def handler(http_request: httpx.Request) -> httpx.Response:
        if http_request.url.path == "/prompt":
            return httpx.Response(200, json={"prompt_id": "provider_restarted"})
        if http_request.url.path == "/history/provider_restarted":
            return httpx.Response(200, json={})
        if http_request.url.path == "/queue":
            return httpx.Response(200, json={"queue_running": [], "queue_pending": []})
        raise AssertionError(f"Unexpected request: {http_request.method} {http_request.url.path}")
    ...
    provider_job_id = await provider.submit(request())

    for _ in range(5):
        assert (await provider.poll(provider_job_id)).status == "queued"
    restarted = await provider.poll(provider_job_id)

    assert restarted.status == "failed"
    assert restarted.error is not None
    assert restarted.error.code == "provider_restarted"
    assert restarted.error.retryable is True
    assert restarted.error.message == "The inference provider restarted before this job completed."
